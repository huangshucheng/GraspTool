        ...
        public async Task<string> ExecuteAsyc(Method method)
        {
            ...
            else if (method == Method.POST)
            {
                ...
                if (!string.IsNullOrEmpty(_customePostData)) {
                    ...
                    //TODO 会卡住 HCC
                    /*
                    _request.BeginGetRequestStream(new AsyncCallback(async (IAsyncResult asynchronousResult)  => {

                        HttpWebRequest requestTmp = (HttpWebRequest)asynchronousResult.AsyncState;
                        Stream postStream = requestTmp.EndGetRequestStream(asynchronousResult);

                        byte[] postData = _postEncoding.GetBytes(_customePostData);
                        postStream.Write(postData, 0, postData.Length);
                        postStream.Close();
                        //await submitRequestAsyc(_request);
                    }), _request);
                    */
